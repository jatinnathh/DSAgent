...
function ReportsView() {
  ...
  const handleEmail = async (report: any) => {
    setEmailingId(report.id);
    try {
      const res = await fetch(`/api/reports/${report.id}/email`, { method: "POST" });
      const data = await res.json();
      if (data.success) {
        setEmailMsg(prev => ({ ...prev, [report.id]: `Sent to ${data.sentTo}` }));
        fetchReports();
      } else {
        setEmailMsg(prev => ({ ...prev, [report.id]: data.error || "Failed" }));
      }
    } catch (e: any) {
      setEmailMsg(prev => ({ ...prev, [report.id]: e.message }));
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 18 }}>
        <div>
          <h2 style={{ fontFamily: C.head, fontSize: "1.15rem", fontWeight: 700, color: C.text, marginBottom: 4, letterSpacing: "-0.02em" }}>Reports</h2>
          <p style={{ fontSize: 12, color: C.textSub, margin: 0 }}>
            PDF reports generated by autonomous pipeline runs.
          </p>
        </div>
        <button onClick={fetchReports} style={{ padding: "6px 14px", borderRadius: 7, border: `1px solid ${C.borderMd}`, background: "transparent", color: C.textSub, fontSize: 11, cursor: "pointer", fontFamily: C.sans }}>
          ↻ Refresh
        </button>
      </div>
      ...
      {loading ? (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {[1, 2].map(i => (
            <div key={i} style={{ height: 80, borderRadius: 12, background: C.card, border: `1px solid ${C.border}`, opacity: 0.3 + i * 0.15 }} />
          ))}
        </div>
      ) : reports.length === 0 ? (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
          style={{ textAlign: "center", padding: "70px 20px", background: C.card, borderRadius: 16, border: `1px solid ${C.border}` }}>
          <div style={{ fontSize: 38, marginBottom: 14 }}>📄</div>
          <div style={{ fontFamily: C.head, fontSize: "1rem", fontWeight: 600, color: C.text, marginBottom: 7 }}>No reports yet</div>
          <p style={{ fontSize: 12, color: C.textSub, maxWidth: 340, margin: "0 auto" }}>
            Run an Autonomous Pipeline to generate a detailed PDF report with EDA, visualizations, and model evaluation.
          </p>
        </motion.div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {reports.map((report) => {
            ...
            const isEmailing = emailingId === report.id;
            const msg = emailMsg[report.id];
            return (
              <motion.div key={report.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}
                ...
                whileHover={{ borderColor: `${C.cyan}44` } as any}
              >
                {/* Icon */}
                <div style={{
                ...
                {/* Info */}
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontFamily: C.head, fontSize: 13, fontWeight: 600, color: C.text, marginBottom: 3, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {report.title}
                  </div>
                  <div style={{ fontSize: 10, color: C.textMute, fontFamily: C.mono, display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    {report.emailSent && (
                      <>
                        <span style={{ color: C.border }}>·</span>
                        <span style={{ color: C.green }}>📧 Emailed</span>
                      </>
                    )}
                    {meta?.phases && (
                      <>
                        <span style={{ color: C.border }}>·</span>
                        <span>{(meta.phases as string[]).length} phases</span>
                      </>
                    )}
                  </div>
                  {report.description && (
                    <div style={{ fontSize: 10, color: C.textSub, marginTop: 4, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: 400 }}>
                      {report.description}
                    </div>
                  )}
                  {msg && <div style={{ fontSize: 10, color: C.green, marginTop: 3, fontFamily: C.mono }}>{msg}</div>}
                </div>

                {/* Actions */}
                <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                  <button onClick={() => handleDownload(report)} style={{
                    padding: "6px 12px", borderRadius: 6, border: "none",
                    background: `linear-gradient(135deg, ${C.cyan}, #0099CC)`, color: "#030712",
                    fontSize: 10, fontWeight: 700, fontFamily: C.head, cursor: "pointer",
                    display: "flex", alignItems: "center", gap: 4,
                  }}>
                    📥 Download
                  </button>
                  <button onClick={() => handleEmail(report)} disabled={isEmailing || report.emailSent} style={{
                    padding: "6px 12px", borderRadius: 6,
                    border: `1px solid ${report.emailSent ? C.green + "40" : C.borderMd}`,
                    background: report.emailSent ? `${C.green}10` : "transparent",
                    color: report.emailSent ? C.green : C.text,
                    fontSize: 10, fontWeight: 600, fontFamily: C.head, cursor: report.emailSent ? "default" : "pointer",
                    opacity: isEmailing ? 0.5 : 1,
                  }}>
                    {report.emailSent ? "✓ Sent" : isEmailing ? "…" : "📧 Email"}
                  </button>
                  <button onClick={() => handleDelete(report.id)} style={{
                    padding: "6px 8px", borderRadius: 6, border: `1px solid ${C.border}`,
                    background: "transparent", color: C.textMute, fontSize: 12, cursor: "pointer",
                  }}
                    onMouseEnter={e => (e.currentTarget.style.color = C.red)}
                    onMouseLeave={e => (e.currentTarget.style.color = C.textMute)}
                  >🗑</button>
