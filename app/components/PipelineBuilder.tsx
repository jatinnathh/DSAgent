                      ...
                      {[...entries].reverse().map((entry, idx, arr) => {
                        ...
                        return (
                          <div key={entry.id} style={{ display: "flex", gap: 10, position: "relative", paddingBottom: isLast ? 4 : 0 }}>
                            ...
                            <div style={{ width: 28, height: 28, borderRadius: "50%", background: entry.status === "error" ? `${C.red}18` : entry.status === "running" ? `${cfg.color}28` : `${cfg.color}15`, border: `1px solid ${entry.status === "error" ? C.red + "55" : entry.status === "running" ? cfg.color + "77" : cfg.color + "33"}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, position: "relative", zIndex: 1, marginTop: 8, flexShrink: 0 }}>
